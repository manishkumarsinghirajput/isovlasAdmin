...
const Banner = () => {
  ...
  const getalltask = () => {
    http
      .post(`/api/v1/admin/banner/list`)
      .then((res) => {
        setData(res.data.data.banners); // Update data with res.data.data.products
      })
      .catch((err) => {
  ...
  const searchHandler = (e) => {
    ...
    http
      .post(`/api/v1/admin/banner/userBanner`, {
        name: value,
      })
      .then((res) => {
  ...
  const deleteHandler = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        http
          .post(`/api/v1/admin/banner/remove`, { id: _id }) // Send user ID in the request body
          .then(() => {
            getalltask();
                ...
                {currentData.map((banners, index) => (
                  ...
                  <tr key={banners._id}>
                    ...
                    <td>
                      {/* <td>{banners.image}</td> */}
                      <td>{<img
                        alt={`Image for ${banners.image}`}
                        style={{ height: "50px", width: "50px" }}
                        src={banners.image} // Only show the first image
                        className="rounded-circle mr-1"
                      />}</td>
                      {/* {banners.images.map((image, index) => (
          <img key={index} src={image.url} alt={`Image ${index + 1}`}  style={{ height: "50px", width: "50px" }} />
        ))} */}
                    </td>

                    <td>{new Date(banners.createdAt).toLocaleDateString()}</td>
        ...
        />
                    ...
                    <td>
                      <>
                        ...
                        <Link to={`/bannerview/${banners._id}`}
                          className="btn px-2 py-1 btn_hover btn-outline-success">
                          <i className="fas fa-eye"></i>
                        </Link>
                        <button
                          onClick={() => deleteHandler(banners._id)}
