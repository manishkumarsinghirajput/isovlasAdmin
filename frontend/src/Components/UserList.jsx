...
const UserList = () => {
  ...
  const getalltask = () => {
    http
      ...
      .then((res) => {
        setData(res.data.data.users); // Update data with res.data.data.users
      })
      .catch((err) => {
  ...
  const deleteHandler = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        http
          .post(`/api/v1/users/remove`, { id: _id }) // Send user ID in the request body
          .then(() => {
            getalltask();
